...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  formData.set('Website', normalizeUrl(formData.get('Website'), FALLBACK_URL));
  formData.set('Website1', normalizeUrl(formData.get('Website1'), FALLBACK_URL));
  ...
  const returnUrl = normalizeUrl(formData.get('Website'), FALLBACK_URL);
  const redirectUrl = new URL(returnUrl);
  redirectUrl.searchParams.set('lead', 'ok');
  redirectUrl.hash = 'registro';
